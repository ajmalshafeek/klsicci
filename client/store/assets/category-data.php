...
function productStoreList($cat, $title)
{
    ...
    $products = '';
    foreach ($dataList as $data) {
        $products .= '<div class="col-6 col-sm-6 col-md-4 col-lg-4 col-xl-4 product-card">
                            <div class="">
...
/*if (isset($_GET['page'])) {
    ...
    foreach ($dataList as $data) {
if($listView==1){
    $products .= '<div class="col-12 listView product-card">';
    $products .= '<div class="row" id="item' . $countProduct . '">';
    $products .= '<div class="col-sm-2">';
    $products .='<a class="product-image"
                                        href="https://' . $_SERVER['HTTP_HOST'] . $config['appRoot'] . '/client/store/product.php?pid=' . $data['id'] . '"><img
                                            src="https://' . $_SERVER['HTTP_HOST'] . $config['appRoot'] . $data['img1'] . '" alt="product" height="64px"/></a>';
    $products .='</div>';
    $products .='<div class="col-sm -3">';
    $products .='<h5 class="product-name"><a
    ...
    $products .='</div>';
    $products .='<div class="col-sm-3">';
    $products .='<p class="product-description">';
    ...
    $products .='<div class="col-sm-2">';
    $products .='<div class="product-action"><button class="action-cart addtocart' . $countProduct . '" title="Add to Cart" onclick="cart(\'item' . $countProduct .
    ...
    $products .='</div>';
        ...
        $products .= '<div class="col-6 col-sm-6 col-md-4 col-lg-4 col-xl-4 product-card">
                            <div class=""  id="item' . $countProduct . '">
                                <figure class="product-media">
                                    <!--<div class="product-label-group"><label
                                            class="product-label label-new">new</label><label
                                            class="product-label label-off">-10%</label>
...
function getBannerList()
{
    ...
    $dataList = fetchBannerList($con, $date);
    return $dataList;
}
