...
function generateInvoicePDF($myOrgName,$orgPhone,$orgFaxNo,$customerName,$attention,$customerAddress,$myOrgAddress,$quotationNumber,$quotationDate,$dueDate,$quotationTotalAmount,$itemList,$quotConfig,$footerContent,$ponumber,$pref,$discount){
	$config=parse_ini_file(__DIR__."/../../jsheetconfig.ini");
/*
...
// debug logo pdf
	...
	if($quotConfig['isLogo']){
		...
		if (@file_exists($LogoSrc)) {
			...
			$myOrLogo='<img style="width:150%;margin-top:-60px;max-height: 150px;" src="'.$LogoSrc.'" alt="logo" />';
		}
	}	else {

		if (@file_exists($orgLogoSrc)) {

			$myOrLogo='	<img  style="width:150%;margin-top:-60px;max-height: 150px;"  src="'.$orgLogoSrc.'" alt="logo" />';
		}
...
$quotTableOrgHeader='
	...
	<tbody>
		...
		<tr>
			...
			if(!empty($ponumber)){
				$quotTableHeader=$quotTableHeader.'<tr class="meta">
				<td rowspan="4">
...
if(!empty($pref)){
    $quotTableHeader=$quotTableHeader.'<tr>
				<td class="meta-head">
					Project Ref.
				</td>
				<td class="meta-body">
    ...
    $quotTableHeader.=$pref.'
                    ...
                    $quotTableHeader.='<td class="meta-head">
					PAYMENT TERM
				</td>
				<td class="meta-body">
