...
class Google_HttpStreamIO extends Google_IO {
   ...
   *
   * @param Google_HttpRequest $request the https request to be executed
   * @return Google_HttpRequest https request with the response https code,
  ...
  public function makeRequest(Google_HttpRequest $request) {
    ...
    $requestHttpContext = array_key_exists('https', $default_options) ?
        $default_options['https'] : array();
    ...
    $options = array("https" => array_merge(self::$DEFAULT_HTTP_CONTEXT,
                                                 $requestHttpContext),
