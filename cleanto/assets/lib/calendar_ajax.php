	...
	jQuery('.ct-tooltipss-load').tooltipster({
	 ...
	 <div class="calendar-body">
	  ...
	  for($day = 1; $day <= $daysInMonth; $day++)
	  {
		...
		<div  title="<?php if($s_date < $c_date){}else{ echo $available_text;} ?>" class=" <?php  if($s_date < $c_date){}else{ echo "ct-tooltipss-load ";}?> ct-week 
		<?php   //if($c_date == $s_date){ echo 'by_default_today_selected'; } ?> 
		<?php  if($s_date < $c_date){ echo 'hide_previous_dates'; }else{ echo 'selected_datess'.$selected_dates;  echo ' remove_selection selected_date';} ?>"  data-id="<?php if($day < 35){echo $k+7; }else{echo $k;} ?>" data-selected_dates="<?php echo $selected_dates; ?>" data-cur_dates="<?php echo $cur_dates; ?>" data-c_date="<?php echo $c_date; ?>" data-s_date="<?php echo $s_date; ?>" 
		
		style="<?php 
		/* background color for full slot
		mookesh
		*/
			if($available_text ==  $label_language_values['none_available'])
			{ 
				echo 'background:orange;cursor: not-allowed;'; 
			} else{
				if($c_date == $s_date){ echo 'by_default_today_selected'; }
			}
		?>">

		<a href="javascript:void(0)"><span><?php echo $day; ?></span></a></div>
		<?php 
	  }
	  if($ct_calendar_firstDay == '1'){
		  while( (($day-1) + $offset) <= $rows * 7)
					...
					{ 
						...
						<?php 
							...
							<li class="time-slot br-2 time_slotss" data-slot_date_to_display="<?php echo str_replace($english_date_array,$selected_lang_label,date($date_format,strtotime($_POST["selected_dates"]))); ?>" data-ct_date_selected="<?php echo  str_replace($english_date_array,$selected_lang_label,date('D, j F, Y',strtotime($_POST["selected_dates"]))); ?>"  data-slot_date="<?php echo $_POST["selected_dates"]; ?>" data-slot_time="<?php echo $slot_time; ?>" data-slotdb_time="<?php echo $slotdbb_time; ?>" data-slotdb_date="<?php echo date('Y-m-d',strtotime($_POST["selected_dates"])); ?>" data-ct_time_selected="<?php echo $ct_time_selected; ?>">
								<?php  
									/*
										Hydrokleen global, hk. slot by section
										mookesh
									*/
									
									if(date("H",strtotime($slot))<12){
										echo "MORNING";
									}else {
										echo "EVENING";
									}
									
									// commend above  and umcomment below to show time
									//if($setting->get_option('ct_time_format')==24){echo date("H:i",strtotime($slot));}else{echo date("h:i A",strtotime($slot));}
								?>
							</li>
						<?php  
						} $slot_counter++; 
