...
class Services_Twilio_HttpStream {
    ...
    public function __call($name, $args) {
        ...
        if (isset($req_body) && strlen($req_body) > 0) {
            $request_options['https']['content'] = $req_body;
        }

        foreach($req_headers as $key => $value) {
            $request_options['https']['header'] .= sprintf("%s: %s\r\n", $key, $value);
        }

        if (isset($this->auth_header)) {
            $request_options['https']['header'] .= $this->auth_header;
        }

        $request_options['https']['method'] = strtoupper($name);
        $request_options['https']['ignore_errors'] = true;
