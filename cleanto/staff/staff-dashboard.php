									...
									<ul class="list-unstyled" id="ct-staff-breaks">
										...
										for ($j = 1; $j <= 7; $j++) {
											...
											<li class="active">
												...
												{?>
													<span class="col-sm-2 col-md-2 col-lg-2 col-xs-12">
	<a id="ct-add-staff-breaks" data-staff_id="<?php echo $_SESSION['ct_staffid']; ?>" data-weekid="<?php echo $i;?>" data-weekday="<?php echo $j;?>"
							...
							<table id="staff-bookings-table" class="display responsive nowrap table table-striped table-bordered" cellspacing="0" width="100%">
								...
								if(sizeof($staff_service_details) > 0){
									...
									<tr>
											...
											echo str_replace($english_date_array,$selected_lang_label,date($getdateformat,strtotime($arr_staff['booking_date_time'])));?> <?php  echo date($timess,strtotime($arr_staff['booking_date_time'])); ?></td>
											...
											?>
											<a name="" class="btn btn-success ct-btn-width mt-20" disabled <?php echo $label_language_values['accepted'];?>><?php echo $label_language_values['accepted'];?></a>
											<?php }else{	 ?>
											<a id="accept_appointment" data-id="<?php echo $arr_staff['order_id'];?>" data-idd=<?php echo $status_insert_id;?> data-status='A'  value="" name="" class="btn btn-info ct-btn-width mt-20" type="submit" title="<?php echo $label_language_values['accept'];?>"><?php echo $label_language_values['accept'];?></a>
											<a id="decline_appointment" data-id="<?php echo $arr_staff['order_id'];?>" data-idd=<?php echo $status_insert_id;?> data-status='D'  value="" name="" class="btn btn-danger ct-btn-width mt-20" type="submit" <?php echo $label_language_values['decline'];?>><?php echo $label_language_values['decline'];?></a>
											<?php } ?>
											</td>
											<td>
												<?php
												...
												{
												?>
												<a name="" class="btn btn-success ct-btn-width mt-20" disabled <?php echo $label_language_values['completed'];?>><?php echo $label_language_values['completed'];?></a>
												<?php
												} else if($today_date >= $book_date && $rec_status_details=='A'){
												?>
												<a id="payment_status" data-toggle="popover"  class="btn btn-info ct-btn-width mt-20" rel="popover" data-placement='left'><?php echo $label_language_values['paid'];?></a> <?php  }  ?>
