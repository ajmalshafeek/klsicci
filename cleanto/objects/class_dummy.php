...
class cleanto_dummy{
	...
	public function add_services_method_unit(){
		$query="insert into `".$this->service_methods_units."` (`id`,`services_id`,`methods_id`,`units_title`,`base_price`,`maxlimit`,`status`, `position`,`limit_title`,`half_section`) values(NULL,'".$this->smu_service_id."','".$this->smu_methods_id."','".$this->smu_units_title."','".$this->smu_base_price."','".$this->smu_maxlimit."','".$this->smu_status."', '0','','D')";
		$result=mysqli_query($this->conn,$query);
		$value=mysqli_insert_id($this->conn);
		return $value;
	}
