...
class cleanto_adminprofile {
    ...
    public function update_password2(){
        if(isset($_SESSION['fp_admin'])){
            $query = "update `".$this->tablename."`  set `password`='".md5($this->password)."'  where `user_email`='".$this->email."'";
            $result=mysqli_query($this->conn,$query);
            return $result;
        }
        elseif(isset($_SESSION['fp_user'])){
            $query = "update `".$this->tablename_user."` set `user_pwd`='".md5($this->password)."'  where `user_email`='".$this->email."'";
            $result=mysqli_query($this->conn,$query);
            return $result;
        }
