...
<head>
	...
	require_once($_SERVER['DOCUMENT_ROOT'] . $config['appRoot'] . "/phpfunctions/invoiceC1.php");
	require_once($_SERVER['DOCUMENT_ROOT'] . $config['appRoot'] . "/phpfunctions/organization.php");
...
<script>
	...
	$(document).ready(function() {
		<?php if (isset($_SESSION['editType'])) {
			...
			foreach ($invoiceBreakdown as $item) {
				...
				echo "cols += `<td><input type=\"text\" class=\"form-control\" value=\"" . $item['itemNo'] . "\" name=\"itemNo' + counter + '\"/></td>`; \n";
				echo "cols += `<td><input type=\"date\" class=\"form-control\" value=\"" . $item['itemDate'] . "\" name=\"itemDate' + counter + '\"/></td>`; \n";
                echo "cols += `<td><input type=\"text\" class=\"form-control\"  value=\"" . $item['itemVehi']. "\"  name=\"itemVehi' + counter + '\" /></td>`; \n";
				...
				echo "cols += `<td><input type=\"number\" min=\"0\" max=\"20000000\" class=\"form-control\" value=\"" . $item['discount'] . "\" name=\"itemDiscount' + counter + '\"/></td>`; \n";
		...
		function fillValueToQuotForm() {
			...
			$('table.order-list > tbody  > tr').each(function() {
				...
				var itemNo = fields.eq(0).val();
				var itemDate = fields.eq(1).val();
                var itemVehi = fields.eq(2).val();
				var itemName = fields.eq(3).val();
				var itemDesc = fields.eq(4).val();
				var itemCost = fields.eq(5).val();
				itemCost = itemCost.replace(/,/g, "");
				var itemQty = fields.eq(6).val();
				var itemDisc = fields.eq(7).val();
				var price = (itemCost * itemQty)-itemDisc;
				...
				cols += '<td valign="top"><input type="text" hidden readonly name="itemNo' + counter + '" value="' + itemNo + '" />' + itemNo + '</td>';
				cols += '<td valign="top"><input type="text" hidden readonly name="itemDate' + counter + '" value="' + itemDate + '" />' + itemDate + '</td>';
                cols += '<td valign="top"><input type="text" hidden readonly name="itemVehi' + counter + '" value="' + itemVehi + '" />' +itemVehi + '</td>';
				...
				cols += '<td valign="top"><input type="number" hidden readonly name="itemDiscount' + counter + '" value="' + itemDisc + '" />' + itemDisc + '</td>';
	...
	function inputForm(str) {
		...
		if ($(window).width() < 991) {
			...
			var itemNo = $(str).find('td').eq(0).find('input').val();
			var itemDate = $(str).find('td').eq(1).find('input').val();
            var itemVehi = $(str).find('td').eq(2).find('input').val();
			var itemName = $(str).find('td').eq(3).find('input').val();
			//	var itemDescription=$(str).find('td').eq(1).find('textarea').val();

			var unitPrice = $(str).find('td').eq(5).find('input').val();
			var quantity = $(str).find('td').eq(6).find('input').val();
			var discount = $(str).find('td').eq(7).find('input').val();

			$('#inputRow_itemNo').prop('value', itemNo);
			$('#inputRow_itemDate').prop('value', itemDate);
            $('#inputRow_itemVehi').prop('value', itemVehi);
			...
			$('#inputRow_itemDiscount').prop('value', discount);
	...
	$(document).ready(function() {
		...
		$('#inputValueToTable').on("click", function() {
			...
			var itemNo = $("#inputRow_itemNo").val();
			var itemDate = $("#inputRow_itemDate").val();
			var itemName = $("#inputRow_itemName").val();
			var itemDescription = $("#inputRow_itemDescription").val();
			var itemVehi = $("#inputRow_itemVehi").val();
			var unitPrice = $("#inputRow_unitPrice").val();
			var quantity = $("#inputRow_qty").val();
			var discount = $("#inputRow_itemDiscount").val();

			$("#" + rowId + "").find('td').eq(0).find('input').prop('value', itemNo);
			$("#" + rowId + "").find('td').eq(1).find('input').prop('value', itemDate);
            $("#" + rowId + "").find('td').eq(2).find('textarea').prop('value', itemVehi);
			$("#" + rowId + "").find('td').eq(3).find('input').prop('value', itemName);
			$("#" + rowId + "").find('td').eq(4).find('textarea').prop('value', itemDescription);

			$("#" + rowId + "").find('td').eq(5).find('input').prop('value', unitPrice);
			$("#" + rowId + "").find('td').eq(6).find('input').prop('value', quantity);
			$("#" + rowId + "").find('td').eq(7).find('input').prop('value', discount);
		...
		$("#addrow").on("click", function() {
			...
			//	cols += '<td>'+counter+'</td>';
			cols += '<td><input type="text" class="form-control" name="itemNo' + counter + '"/></td>';
			cols += '<td><input type="date" class="form-control" name="itemDate' + counter + '"/></td>';
            cols += '<td><input type="text" class="form-control" id="inputVehi' + counter + '" " name="itemVehi' + counter + '"/></td>';
			...
			cols += '<td><input type="number" min="0" max="2000000" class="form-control" name="itemDiscount' + counter + '"/></td>';
			...
			<div class="tab-content" id="pills-tabContent">
				...
				<div class="tab-pane fade show active " id="pills-clientDetails" role="tabpanel" aria-labelledby="pills-clientDetails-tab">
					...
					<div class="form-group row prefstatus" style="display:none">
						<label for="prefnumber" class="col-sm-2 col-form-label col-form-label-lg">D/O TERM:</label>
						<div class="col-sm-10">
						...
						<table class="table order-list table-responsive  table-hover table-bordered" style="width:100%" id="tab_logic">
							<thead>
								<tr>
									...
									<th class="text-center" class="th_item" style="color:#19334d;width:10%">
										No:
									</th><th class="text-center" class="th_item" style="color:#19334d;width:10%">
										Date:
									</th>
                                    <th class="text-center" class="th_vehi" style="color:#19334d;width:10%">
                                        Vehicle No:
                                    </th>
                                    <th class="text-center" class="th_item" style="color:#19334d;width:10%">
										Item Code:
									</th>
									<th class="text-center" class="th_desc" style="color:#19334d;width:20%">
										Description:
									</th>
									<th class="text-center" class="th_cost" style="color:#19334d;width:13%">
										Unit Cost:
									</th>
							...
							<tbody>
								<tr id='inputRow_' onclick="inputForm(this)">
									...
									<td>
										<input type="text" name='itemNo0' class="form-control" />
									</td>
                                    <td>
										<input type="date" name='itemDate0' class="form-control" />
									</td>
                                    <td>
                                        <input type="text" name='itemVehi0' class="form-control" />
                                    </td>
                                    <td>
									...
									<td>
										<input type="number" name='itemDiscount0' min="0" max="20" class="form-control" />
									</td>
									<!--
									<td>
				...
				<div class="tab-pane fade " id="pills-confirm" role="tabpanel" aria-labelledby="pills-confirm-tab">
					...
					<form id="invoiceCreationForm" action="<?php echo 'https://' . $_SERVER['HTTP_HOST'] . $config['appRoot'] . "/phpfunctions/invoiceC1.php"; ?>" method="POST">
						<div class="form-group row">
						...
						<td class="meta-body">
                            <input type="date" readonly value="" id="quot_quotationDate" name="quot_quotationDate" />
						</td>
						</tr>
					        <tr>
							<td class="meta-head">
							...
							<td class="meta-body">
                                <input type="date" id="quot_dueDate" value="" class="dueDate" name="quot_dueDate" />
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="meta-body">
                                                        <input type="text" id="quot_mod" value="" class="mod" name="quot_mod" />
                                                    </td>
                                                </tr>
						</table>
						</td>
						...
						<table class="quotation-Content">
							<thead>
								<tr>
									<th style="width:2%;min-width:10px">
										No:
									</th>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div id='inputFormModalContent'>
						...
						<div class="form-group row">
							<label for="inputRow_itemNo" class="col-sm-2 col-form-label col-form-label-md">ITEM No</label>
							<div class="col-sm-10">
								<input type="number" class="form-control" placeholder="Item no" id="inputRow_itemNo" />
							</div>
						</div>
						<div class="form-group row">
						...
						<div class="form-group row">
							<label for="inputRow_itemName" class="col-sm-2 col-form-label col-form-label-md">ITEM NAME</label>
							<div class="col-sm-10">
						...
						<div class="form-group row">
							<label for="inputRow_itemDescription" class="col-sm-2 col-form-label col-form-label-md">ITEM DESCRIPTION</label>
							<div class="col-sm-10">
						...
						<div class="form-group row">
							<label for="inputRow_itemVehi" class="col-sm-2 col-form-label col-form-label-md">Vehicle No</label>
							<div class="col-sm-10">
								<input type="text" class="form-control" placeholder="Vehicle No" id="inputRow_itemVehi" />
							</div>
						</div>

						<div class="form-group row">
							<label for="inputRow_unitPrice" class="col-sm-2 col-form-label col-form-label-md">UNIT PRICE</label>
							<div class="col-sm-10">
						...
						<div class="form-group row">
							<label for="inputRow_qty" class="col-sm-2 col-form-label col-form-label-md">QUANTITY</label>
							<div class="col-sm-10">
						...
						<div class="form-group row">
							<label for="inputRow_discount" class="col-sm-2 col-form-label col-form-label-md">DISCOUNT</label>
							<div class="col-sm-10">
								<input type="number" class="form-control" placeholder="Discount" id="inputRow_discount" />
							</div>
						</div>
					</div>
