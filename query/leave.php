...
/*
function addDepartment($con,$departmentName){
    $success=false;
	$status=0;
    //debug
...
function applyLeave($con,$leaveType,$toDate,$fromDate,$description,$status,$isRead,$empId,$num_days,$postingDate){
	$success=false;
	//debug
	//$role=$role;

	$query="INSERT INTO tblleaves(LeaveType,ToDate,FromDate,Description,Status,IsRead,empid,num_days,PostingDate) VALUES(?,?,?,?,?,?,?,?,?)";
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'sssssiids',$leaveType,$toDate,$fromDate,$description,$status,$isRead,$empId,$num_days,$postingDate);
	if(mysqli_stmt_execute($stmt)){
		$success=true;
		mysqli_stmt_close($stmt);
	}
...
function getLeaveType($con){
	$query="SELECT leave_name from leave_type WHERE ?";
	$dataList=array();
	$stmt=mysqli_prepare($con,$query);
	$temp=1;
...
function listLeaves($con,$type,$check1,$check2,$userid){
	$temp="";
	$userRestriction="";
	if($check1 || $check2){
		$userRestriction;
		...
		$userRestriction=" AND empid=".$userid;
	}
	if($type==-1) {
		$temp = "leave_access>?";
	}
	...
	elseif ($type==2){
		$temp = "leave_access=? AND admin_status='Approved'";
	}
	elseif ($type==3){
		$temp = "leave_access<? AND admin_status='REJECTED'";
	}
	//$query="SELECT `leave`.id AS lid, `org`.fullName, `org`.staffId, `leave`.LeaveType, `leave`.PostingDate, `leave`.Status, `leave`.admin_status FROM `tblleaves` AS `leave` JOIN `organizationuser` AS `org` WHERE `org`.leave_access=? ORDER BY lid DESC";
	$query="SELECT `id` AS `lid`, `LeaveType`, `PostingDate`, `Status`, `admin_status`, `empid` FROM `tblleaves` AS `leave` WHERE ".$temp.$userRestriction." ORDER BY lid DESC";
	$dataList=array();
	$stmt=mysqli_prepare($con,$query);
...
function getStaffFullName($con, $id){
	$query="SELECT `fullName` FROM `organizationuser` WHERE `id`=?";
	$dataList=array();
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'i',$id);
	mysqli_stmt_execute($stmt);
...
function getStaffID($con, $id){
	$query="SELECT `staffId` FROM `organizationuser` WHERE `id`=?";
	$dataList=array();
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'i',$id);
	mysqli_stmt_execute($stmt);
...
function checkleave1LevelModule($con, $id)
{
	$query="SELECT `module`.`moduleId` FROM `rolemodules` AS `module` JOIN `organizationuser` AS `org` WHERE `org`.`role`=`module`.`roleId` AND `org`.`id`=? AND `module`.`moduleId`=49";
	$dataList=false;
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'i',$id);
	mysqli_stmt_execute($stmt);
...
function checkleave2LevelModule($con, $id)
{
	$query="SELECT `module`.`moduleId` FROM `rolemodules` AS `module` JOIN `organizationuser` AS `org` WHERE `org`.`role`=`module`.`roleId` AND `org`.`id`=? AND `module`.`moduleId`=48";
	$dataList=false;
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'i',$id);
	mysqli_stmt_execute($stmt);
...
function getLeaveSelectedDetails($con, $lid){
	//$query='SELECT tblleaves.id as lid,organizationuser.fullName,organizationuser.staffId,organizationuser.phone,organizationuser.email,organizationuser.al_entitle,organizationuser.mc_entitle,tblleaves.LeaveType,tblleaves.ToDate,tblleaves.FromDate,tblleaves.Description,tblleaves.PostingDate,tblleaves.Status,tblleaves.AdminRemark,tblleaves.admin_status,tblleaves.registra_remarks,tblleaves.AdminRemarkDate,tblleaves.num_days from tblleaves join organizationuser on tblleaves.empid=organizationuser.staffId where tblleaves.id=:lid';
	$query="SELECT `id` as `lid`,`LeaveType`,`ToDate`,`FromDate`,`Description`,`PostingDate`,`Status`,`AdminRemark`,`admin_status`,`registra_remarks`,`AdminRemarkDate`,`num_days`,`empid` FROM `tblleaves` WHERE `id`=?";
	$dataList=array();
	$stmt=mysqli_prepare($con,$query);
...
function getLeaveOrgDetail($con, $id){
	//`fullName`, `staffId`, `phone`, `email`, `al_entitle`, `mc_entitle`
	$query="SELECT `fullName`, `staffId`,`staffDesignation`, `phone`, `email`, `al_entitle`, `mc_entitle` FROM `organizationuser` WHERE `id`=?";
	$dataList=array();
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'i',$id);
	mysqli_stmt_execute($stmt);
...
function getListLeaveOrgDetail($con, $id){
	//`fullName`, `staffId`, `phone`, `email`, `al_entitle`, `mc_entitle`
	$query="";
	if($_SESSION['ManagerRole'] || $_SESSION['role']==1 || $_SESSION['role']==42){
		$query="SELECT `id`, `fullName`, `staffId`, `al_entitle`, `mc_entitle` FROM `organizationuser` WHERE `role` NOT IN (42,1) AND `id` NOT IN (?)";
	}else{
		$query="SELECT `id`,`fullName`, `staffId`, `al_entitle`, `mc_entitle` FROM `organizationuser` WHERE `id`=?";
	}
	$dataList[]=array();
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'i',$id);
...
function updateLevel1Leave($con,$description,$status,$admremarkdate,$did){
	$temp=false;
	$leave_access=1;
	$query="update `tblleaves` set `AdminRemark`=?,`Status`=?,`AdminRemarkDate`=?,`leave_access`=? where `id`=?";
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'ssssi',$description,$status,$admremarkdate,$leave_access,$did);
	if(mysqli_stmt_execute($stmt)){
		$temp=true;
	}
	return $temp;
}
function updateLevel2Leave($con,$admremarkdate,$reg_remarks,$reg_status,$did){
	$temp=false;
	$leave_access=2;
	$query="update `tblleaves` set `AdminRemarkDate`=?, `registra_remarks` = ?, `admin_status` = ?,`leave_access`=? where `id`=?";
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'ssssi',$admremarkdate,$reg_remarks,$reg_status,$leave_access,$did);
	if(mysqli_stmt_execute($stmt)){
		$temp=true;
	}
	return $temp;
}
function updateLeave($con, $al_entitle, $mc_entitle, $empId){
	$temp=false;
	$query="update `organizationuser` set `al_entitle`=?, `mc_entitle` = ? where `id`=?";
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'ddi',$al_entitle,$mc_entitle,$empId);
	if(mysqli_stmt_execute($stmt)){
		$temp=true;
	}
	return $temp;
}
function addLeave($con,$al_entitle,$mc_entitle){
	$temp=false;
	$empId=0;
	$query="update `organizationuser` set `al_entitle`=`al_entitle`+?, `mc_entitle` =`mc_entitle`+? where `id`>?";
	$stmt=mysqli_prepare($con,$query);
	mysqli_stmt_bind_param($stmt,'ddi',$al_entitle,$mc_entitle,$empId);
	if(mysqli_stmt_execute($stmt)){
		$temp=true;
...
function removeLeave($con,$al_entitle,$mc_entitle){
	$temp=false;
	$empId=0;
	$query="update `organizationuser` set `al_entitle`=`al_entitle`-?, `mc_entitle` =`mc_entitle`-? where `id`>?";
	$stmt=mysqli_prepare($con,$query);
...
function deleteLeave($con,$lid)
{
	$success = false;

	$query = "DELETE FROM `tblleaves` WHERE id=?";
	$stmt = mysqli_prepare($con, $query);
	mysqli_stmt_bind_param($stmt, 'i', $lid);
	if (mysqli_stmt_execute($stmt)) {
		$success = true;
