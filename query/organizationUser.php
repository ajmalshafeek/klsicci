...
function checkifLoanExist($con,$staffId){
    $res=false;
    $query="SELECT * FROM `staffloan` WHERE sid='".$staffId."'";
    $stmt=mysqli_prepare($con,$query);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    while($row=$result->fetch_assoc()){
...
function checkDesignationName($con,$name){
	$query="SELECT COUNT(role) as num FROM `roles` WHERE `role`=?";
	$stmt=mysqli_prepare($con,$query);
