...
date_default_timezone_set("Asia/Kuala_Lumpur");
	if(isset($_GET['getInvoiceByInvNo'])){
		...
		$invoiceDetails=null;
		require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/query/invoiceC1.php");
	...
	if(isset($_POST['createInvoice'])){
		...
		require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/phpfunctions/pdf/invoicePDFC1.php");
		...
		for( $i=0; $i<$maxItemIndex; $i++ ){
			...
			$itemVehi=$_POST["itemVehi$i"];
			$itemCost=$_POST["itemCost$i"];
			$itemQty=$_POST["itemQty$i"];
			$itemDiscount=$_POST["itemDiscount$i"];
			$price=$_POST["price$i"];

			$item=array("itemNo"=>$itemNo,"itemDate"=>$itemDate,"itemName"=>$itemName,"itemDescription"=>$itemDescription,
			"itemVehi"=>$itemVehi,"itemCost"=>$itemCost,"itemQty"=>$itemQty,"itemDiscount"=>$itemDiscount,"price"=>$price);
		...
		//$quotationNo;
		$invoiceDirectory=$_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/resources/".$orgId."/InvoiceC1/".date("Y/M");
		...
		$total,$amountPaid,$status,$orgId,$fileName,$footerId,$recurring,$recurringDate,$recurringEnd,$recurringBy); // query/invoiceC1.php

		if($invcId>0){
			require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/query/invoiceItemC1.php");
			foreach($itemList as $item){
				$itemNo=$item['itemNo'];
				$itemDate=$item['itemDate'];
				$itemName=$item['itemName'];
				$itemDescription=$item['itemDescription'];
                $itemVehi=$item['itemVehi'];
				...
				$subTotal=null;
				$tax=null;
				$discounti=$itemDiscount;
				$total=$price;

				$itemCost=preg_replace("/[^0-9\.]/", '', $itemCost);
				$total=preg_replace("/[^0-9\.]/", '', $total);

				$saveSucess=createInvoiceItemBreakdownC1($con,$invcId,$itemName,$itemDescription,$itemVehi,$itemCost,$itemQty,$subTotal,$tax,$discounti,$total,$itemNo,$itemDate); // query/quotationitem.php
			}
		}
		if($saveSucess){
		...
		$dueDate,$quotationTotalAmount,$itemList,$quotConfig,$content,$ponumber,$pref,$discount,$mod); // phpfunction/pdf/invoicePDFC1.php
	//	$invoicePDF->output();
...
if($recurring==0) {
    header("Location: https://" . $_SERVER['HTTP_HOST'] . $config['appRoot'] . "/organization/InvoiceC1/mailInvoiceC1.php");
	...
	//	$invoicePDF->output();
		...
		$total,$amountPaid,$status,$fileName,$orgId,$invoiceDetails['id'],$footerId); // query/invoiceC1.php
		if($updateSuccess){
			require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/query/invoiceItemC1.php");
			...
			foreach($itemList as $item){
				...
				$saveSucess=createInvoiceItemBreakdownC1($con,$invoiceDetails['id'],$itemName,$itemDescription,$itemVehi,$itemCost,$itemQty,$subTotal,$tax,$discount,$total,$itemNo,$itemDate); // query/quotationitem.php
			}
		}
	...
	if(isset($_POST['invoiceEditButton'])){
		...
		header("Location:  https://".$_SERVER['HTTP_HOST'].$config['appRoot']."/organization/InvoiceC1/createInvoiceC1.php");
  ...
  if (isset($_POST['revalidateButton'])) {
	...
	function invoiceListTableC1($invoiceId_,$customerName_,$jobId_,$invoiceNo_,$customerId_,$createdBy_,
	$dateFrom_,$dateTo_,$status_,$orgId_){
		...
		$invoiceList=fetchInvoiceListC1($con,$invoiceId,$customerName,$jobId,$invoiceNo,$customerId,
	...
	function getInvoiceBreakdownByInvId($invoiceId_){
		...
		require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/query/invoiceItemC1.php");
		$invoiceBreakdown=fetchInvoiceDetailsByInvoiceIdC1($con,$invoiceId);
		$dataList=array();
	...
	function getInvoiceMailList($invoiceId_){
		...
		require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/query/invoiceMailListC1.php");
	...
	if(isset($_POST['downloadSelectedInvoiceFile'])){
		if (isset($_POST['checkedRow']))
		{
			...
			if(sizeof($_POST['checkedRow'])==1){
				...
				ob_start();
				$file=$_SERVER['DOCUMENT_ROOT'].$config['appRoot'].'/resources/'.$_SESSION['orgId'].'/InvoiceC1/'.$_POST['checkedRow'][0].'.pdf';
				...
				foreach($_POST['checkedRow'] as $fileName){
					array_push($files,$_SERVER['DOCUMENT_ROOT'].$config['appRoot'].'/resources/'.$_SESSION['orgId'].'/InvoiceC1/'.$fileName.'.pdf');
